...
export default function Input2({ label, text, setText, comment, commentClass, type, validate }: Input2Props) {
    ...
    function onBlur(e: FocusEvent<HTMLInputElement>) {
        ...
        if (inputRef.current != null) {
            ...
            // }
            console.log('validationMessage during onBlur', inputRef.current.validationMessage);
        }
        setFocussed(false)
    }
    ...
    return (
        <>
            <div className={styles.box}>
                ...
                <div className={`${commentClass} ${styles.comment}`}>{comment ?? '' + '-' + (inputRef.current?.validationMessage ?? '')}</div>
            </div>
        </>
    )
}